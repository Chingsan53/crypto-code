...
def intTo4bitBin(myInt):
    ...
    if len(binary) > 4:
        print("intTo4bitBin(): error, cannot convert ",
              myInt, " into a 4-bit number")
        exit(1)
...
def doTable(table, inputStr):
    ...
    for tblIndex in table:

        # Add the bit
        output += inputStr[tblIndex]
    return output
...
def roundFunction(rhs, roundKey):
    ...
    # Run the right-hand-half through the E-table
    expandedRHS = doTable(__expansion_table, rhs)

    # XOR the output
    rhsXORKey = strXOR(expandedRHS, roundKey)

    # split the string into 8 6-bit inputs
    sBoxInputs = split48BitString(rhsXORKey)

    # List of S-Boxes
    sBoxList = [__sbox1, __sbox2, __sbox3, __sbox4,
                __sbox5, __sbox6, __sbox7, __sbox8]

    # The S-Box number
    sNum = 0

    # Feed each input into every respective S-Box
    while sNum < 8:

        # Run the input through the S-Box
        Sout += doSbox(sBoxList[sNum], sBoxInputs[sNum])

        # go to the next S-Box
        sNum += 1

    # Run the result through the P-Table
    return doTable(__p, Sout)
...
def deriveSingleRoundKey(roundNum, key):
    lhs = key[1:28]
    rhs = key[28:]

    # get the amount of rotation
    rotAmount = __left_rotations[roundNum]

    return rotateStringLeft(lhs) + rotateStringLeft(rhs)


def deriveAllRoundKeys(key):

    # Run the initial key through PC1
    pc1Out = doTable(__pc1, key)

    # The list of round keys
    keys = []

    # The round number
    roundNum = 0
    # The current left-circular shifted key
    currShiftedKey = pc1Out

    # Derive 16 keys
    while roundNum < 16:

        # Split the key into left and right hand halves
        rhs = currShiftedKey[28:]
        lhs = currShiftedKey[:28]

        # Perform the left-circular shifts on each half
        lhs = rotateStringLeft(lhs, __left_rotations[roundNum])
        rhs = rotateStringLeft(rhs, __left_rotations[roundNum])

        # Put the shifted halves back together
        currShiftedKey = lhs + rhs

        # Get the round key
        roundKey = doTable(__pc2, currShiftedKey)
...
# The plaintext
plaintext = "1010111001100001110001110100111110111001110011000110010000111010"

# The original key
originalKey = "0110000111011101101111011100001110100110101101001010111100101011"

# print(deriveAllRoundKeys(originalKey))

# Round Key list
roundKeyList = deriveAllRoundKeys(originalKey)

# Run the plaintext through the IP Table
ipResult = doTable(__ip, plaintext)
...
while roundNum < 16:
    # Split the plaintext into right and left-hand halves
    lhs = plaintext[:32]
    rhs = plaintext[32:]

    # Run the right-hand half through the round function
    roundFunctionOutR = roundFunction(rhs, roundKeyList[roundNum])

    tmpLhs = rhs

    # XOR the output of the round function against the left-hand half
    rhs = strXOR(roundFunctionOutR, lhs)
    lhs = tmpLhs

    # The plaintext to be fed into the next round
    plaintext = lhs + rhs
...
plaintext = doTable(__fp, plaintext)
...
print("IP(Original Block): " + ipResult)

# Run the original key from PC1
keyFromPC1 = doTable(__pc1, originalKey)

print("Original key: " + originalKey)
print("PC-1(Original key): " + keyFromPC1)
print(len(originalKey), len(keyFromPC1))

# Rotate String
print(rotateStringLeft("abcdfg", 2))
